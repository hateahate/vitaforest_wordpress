...
class WT_Wishlist_Looppage {
    ...
    public function render_webtoffee_wishlist_button() {
        ...
        if ($this->product_already_exists($product->get_id(), get_current_user_id())) {
            ...
            $icon_src       = WEBTOFFEE_WISHLIST_BASEURL .'public/images/icon_unfavourite.svg';
        ...
        if(isset($wt_wishlist_button_style_settings_options['wt_button_type'])){
            ...
            if($wt_wishlist_button_style_settings_options['wt_button_type'] == 'text'){
                ...
                echo "<div class='".$icon_class."'> <a href='#' ><span class='" . $class . " wt-wishlist-button' data-act='add' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "'>".'<img class="wishlist_text_icon_image" style="margin-bottom: -2px !important;" src="'.$text_icon_src.'">'."</span></a> ".$element." </div>";

            }else if($wt_wishlist_button_style_settings_options['wt_button_type'] == 'icon'){

                if($icon_position == 1){
                    echo "<div class='".$icon_class."'> <a href='#' > <i class='" . $class . " wt-wishlist-button' data-act='add' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "'><img style='width:auto; display:inline-flex; margin-bottom: 0px !important;'src='".$text_icon_src."'></i> </a> ".$element." </div>";
                }else{
                    echo "<div class='".$icon_class."'> <a href='#' > <i class='" . $class . " wt-wishlist-button' data-act='add' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "'><img style='width:auto; display:inline-flex; margin-bottom: 0px !important;'src='".$icon_src."'></i> </a> ".$element." </div>";
                }

            }else if($wt_wishlist_button_style_settings_options['wt_button_type'] == 'normal_button'){

                if($product_added == 1){
                    echo "<div class='".$icon_class."'> <a href='#' > <img class='wishlist_text_icon_image' style='margin-bottom: -2px !important;' src='".$text_icon_src."'> <span class='" . $class . " wt-wishlist-button' data-act='add' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "'>".$button_msg."</span></a> ".$element." </div>";
                }else{
                    echo "<div class='".$icon_class."'>  <button  style='margin-top:0px !important' class='button " . $class .$button_class. " wt-wishlist-button' data-act='add' data-product_id='" . $product->get_id() . "' data-user_id='" . get_current_user_id() . "'>".$button_msg."</button> </div>";
                }
            }
        }
