...
function breadcrumb_permalink_product_cat($breadcrumb_items){
    ...
    if(!empty($category_string)){
        ...
        if(is_singular()){
		    ...
		    $preloadUrl = $breadcrumb_items_new['link'];
			$formattedUrl = parse_url($preloadUrl);
			$defaultPath = $formattedUrl['path'];
			$defaultPathtoCheck = substr($defaultPath, 0, 10);
			$urlReplaceMask = '/shop/product-category-';
			$urlsubMask = '/category-';
			if($defaultPathtoCheck === $urlsubMask){
			$correctUrl = str_replace($urlsubMask, $urlReplaceMask, $defaultPath);
			}
			else{
			$urlMask = '/product-category/';
			$urlReplaceMask = '/shop/product-category-';
			$correctUrl = str_replace($urlMask, $urlReplaceMask, $defaultPath);
			}
			$formattedUrl['path'] = $correctUrl;
			foreach($formattedUrl as $key => $value){
				if ($key == 'scheme'){
					$value .='://';
				}
				$finishedUrl .= $value;
			}
			$breadcrumb_items_new['link'] = $finishedUrl;
            $breadcrumb_items = array_merge($breadcrumb_items, $breadcrumb_items_new);
