...
class WpManager
{
    ...
    public function addFilterQueryToWpQuery($wp_query)
    {
            ...
            if( isset( $wp_query->tax_query->queries ) ){
                ...
                if (is_array($tax_query) && !empty($tax_query)) {
                    foreach ($tax_query as $value) {
                        $new_tax_query[] = $this->addTaxQueryArray($value);
                    }

                    if (!empty($new_tax_query)) {
                        $wp_query->set('tax_query', $new_tax_query);
                    }
                }
            }
        ...
        if ( ! empty( $setIds ) && $this->isFilterRequest() ) {
            ...
            foreach ($this->getQueryVar('queried_values') as $queried_value) {
                ...
                if( ! $wp_query->get('flrt_query') && in_array( $queried_value_key, $set_filter_keys ) ){
    ...
    public function excludeChildrenFromTaxQueryClone( $wp_query )
    {
        // Do not include posts from children taxonomy terms
        if( $wp_query->get('flrt_query') ){
    ...
    public function addTaxQueryArray($tax_query_array)
    {
    ...
    private function collectWPQueries( $wp_query )
    {
        if( $wp_query->is_archive() ||
            ...
            if( $wp_query->get('flrt_query') || $wp_query->get('flrt_set_query') ){
                return $wp_query;
            }
