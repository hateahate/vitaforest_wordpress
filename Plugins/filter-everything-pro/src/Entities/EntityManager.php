...
class EntityManager
{
    ...
    */
    ...
    public function getAlreadyFilteredPostIds( $setId, $exceptEntity = false )
    {
        ...
        if( $wpManager->getQueryVar('wpc_is_filter_request') ){
            ...
            if(! empty( $filteredPostsIds ) ) {
                $intersection = $this->getBetweenFiltersIntersect($filteredPostsIds, $allWpQueriedPostIdsKeys);
                $postIds = array_flip($intersection);
            }
        }
    ...
    public function getAllSetWpQueriedPostIds( $setId )
    {
        ...
        if ( $ids === false ) {
            ...
            if ($set_filter_query instanceof \WP_Query) {
                ...
                $set_filter_query->set('flrt_query', true);
    ...
    // It seems this method would be better to fire before output
    public function prepareEntitiesToDisplay( $sets )
    {
        ...
        if( ! $container->getParam( $key ) ) {
            ...
            foreach ($allEntities as $entityName => $entity) {
                ...
                $filter = $this->getFilterBy('e_name', $entityName, array('logic', 'e_name', 'orderby'));
...
//                break;
    ...
    public function calcTermCount( $termPostsIds, $filteredPostsIds, $allPostsIds, $filter )
    {
        ...
        $newIntersect = $this->getBetweenFiltersIntersect( $filteredPostsIds, $allPostsIds );

        return count( array_intersect_key( $newIntersect, $termPostsIds ) );
    }
                ...
                if ( $doCalculate ) {
                    ...
                    // Intersection for logic AND between filter terms
                        ...
                        if( ! empty( $queriedAllPosts[$e_name] ) ){
                            $queriedAllPosts[$e_name] = array_intersect_key( $allPostsIds, $queriedAllPosts[$e_name], array_flip($term->posts) );
                        }else{
                            $queriedAllPosts[$e_name] = array_intersect_key( $allPostsIds, array_flip($term->posts) );
                        }
